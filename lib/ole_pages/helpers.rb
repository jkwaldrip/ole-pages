...
module OlePages
  ...
  module Helpers
    ...
    # Return a random string of letters with the specified length.
    def random_letters(len = 6, str = "")
      len.times { str << pick_range('a'..'z')}
      str
    end

    def random_alphanumerics(len = 8, str = "")
      len.times { str << (('A'..'Z').to_a + ('0'..'9').to_a).sample }
      str
    end
